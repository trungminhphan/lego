...
if($act == 'check'){
    ...
    if($nguoichoi->check()) transfers_to('nguoichoi_1.html?msg=Cập nhật tình trạng thành công');
} else {
	if($hinhanh_file) $hinhanh = $gridfs->insert_files();      
	...
	$nguoichoi->capdo = $capdo;
    $nguoichoi->diem = $diem;
	if($nguoichoi->insert()) transfers_to('nguoichoi_1.html?msg=Thêm thành công');
}

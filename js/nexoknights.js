...
/* global LEGO */
LEGO.logger = function(window) {
    "use strict";
    var instance = {};
    //Create as singleton to ensure it is only create once per page
    var createLogger = function() {
        var _debug = true;
        var writer = window.console || {
            warn: function() {},
            log: function() {},
            error: function() {}
        };
    ...
    var getInstance = function(name) {
        ...
        if (!instance[name]) {
            instance[name] = createLogger();
        }
        return instance[name];
    };
    var setDebug = function(isDebug, name) {
        if (!name) {
            name = "default";
        }
...
LEGO.mini = function() {
  ...
  */
    ...
    var setter = {}, getter = {};
    var idSequence = 1;
    // used as node id to identify nodes, and as general id for other maps
    // @condblock ie8compatibility
    var registeredEvents = {};
    // nodeId -> [handler objects] ; for on()
    // @condend
    /*$
  * @id ready_vars
  * @dependency
  */
    ...
    // check for 'interactive' and 'complete'
    /*$
  * @id animation_vars
  * @dependency
  */
    var animationHandlers = {};
    // global map of id->run() currently active
    var animationHandlerCount = 0;
  ...
  */
    ...
    // template -> function
    var templates = [];
    // list of MAX_CACHED_TEMPLATES templates
    ///#/snippet utilVars
  ...
  */
    ...
    function eachObj(obj, cb, ctx) {
        for (var n in obj) if (obj.hasOwnProperty(n)) cb.call(ctx || obj, n, obj[n]);
        return obj;
    }
    function each(list, cb, ctx) {
        if (list) for (var i = 0; i < list.length; i++) cb.call(ctx || list, list[i], i);
        return list;
    }
    function filter(list, filterFuncOrObject, ctx) {
        ...
        each(list, function(value, index) {
            if (f.call(ctx || list, value, index)) r.push(value);
        });
        return r;
    }
    ...
    function startsWith(base, start) {
        if (isList(base)) {
            var s2 = _(start);
            // convert start as we don't know whether it is a list yet
    ...
    function copyObj(from, to) {
        var dest = to || {};
        for (var name in from) dest[name] = from[name];
        return dest;
    }
    function merge(list, target) {
        var o = target;
        for (var i = 0; i < list.length; i++) o = copyObj(list[i], o);
        return o;
    }
    ...
    function sub(list, startIndex, endIndex) {
        ...
        if (list) {
            var e = getFindIndex(list, endIndex, list.length);
            for (var i = getFindIndex(list, startIndex, 0); i < e; i++) r.push(list[i]);
        }
        return r;
    }
    function array(list) {
    ...
    function uniq(list) {
        ...
        return filter(list, function(item) {
            if (found[item]) return false; else return found[item] = 1;
        });
    }
    ...
    function contains(list, value) {
        // TODO: can Array.indexOf be used in >IE8?
    ...
    function equals(x, y) {
        ...
        var aKeys;
        if (a == b) return true; else if (a == _null || b == _null) return false; else if (isValue(a) || isValue(b)) return isDate(a) && isDate(b) && +a == +b; else if (isList(a)) {
            return a.length == b.length && !find(a, function(val, index) {
                if (!equals(val, b[index])) return true;
            });
        } else {
        ...
        if (isDate(value)) {
            ...
            if (match = /^([<>]?)(=?)([^:]*?)\s*:\s*(.*)$/.exec(fmtPart)) {
                ...
                if (isNaN(cmpVal2) || !isNumber(cmpVal1)) {
                    ...
                    // not ""+value, because undefined is treated as null here
                    cmpVal2 = match[3];
                }
                if (match[1]) {
    ...
    function parseDate(fmt, date) {
        ...
        var match;
        var format = replace(fmt, /^\?/);
        if (format != fmt && !trim(date)) return _null;
        if (match = /^\[([+-]\d\d)(\d\d)\]\s*(.*)/.exec(format)) {
    ...
    function parseNumber(fmt, value) {
        var format = replace(fmt, /^\?/);
        if (format != fmt && !trim(value)) return _null;
    ...
    function createEventHandler(handler, registeredOn, args, index, prefix, selectorFilter) {
        ...
        return function(event, triggerOriginalTarget) {
            ...
            if (selectorFilter) while (el && el != registeredOn && !(match = selectorFilter(el))) el = el["parentNode"];
            if (match && (stop = (!handler.apply($(selectorFilter ? el : registeredOn), args || [ e, index ]) || prefix == "") && prefix != "|") && !triggerOriginalTarget) {
                if (e["preventDefault"]) {
                    // W3C DOM3 event cancelling available?
                    e["preventDefault"]();
                    e["stopPropagation"]();
                }
                e["cancelBubble"] = true;
    ...
    function on(subSelector, eventSpec, handler, args, bubbleSelector) {
        if (isFunction(eventSpec)) return this["on"](_null, subSelector, eventSpec, handler, bubbleSelector); else if (isString(args)) return this["on"](subSelector, eventSpec, handler, _null, args); else return this["each"](function(baseElement, index) {
            flexiEach(subSelector ? dollarRaw(subSelector, baseElement) : baseElement, function(el) {
                flexiEach(toString(eventSpec).split(/\s/), function(namePrefixed) {
                    ...
                    var capture = !!bubbleSelector && (name == "blur" || name == "focus");
                    // bubble selectors for 'blur' and 'focus' registered as capuring!
                    ...
                    if (IS_PRE_IE9) {
                        el.attachEvent("on" + handlerDescriptor.eventType + (capture ? "in" : ""), miniHandler);
                        // IE < 9 version
                        push(registeredEvents, getNodeId(el), handlerDescriptor);
                    } else {
                        el.addEventListener(name, miniHandler, capture);
                        // W3C DOM
    ...
    function off(handler) {
        flexiEach(handler["M"], function(h) {
            if (IS_PRE_IE9) {
                h.element.detachEvent("on" + h.eventType + (h.capture ? "in" : ""), h.handlerFunc);
                // IE < 9 version
                removeFromArray(registeredEvents[h.element[MINIFIED_MAGIC_NODEID]], h);
            } else {
                h.element.removeEventListener(h.eventType, h.handlerFunc, h.capture);
                // W3C DOM
    ...
    function clone(listOrNode) {
        return collector(flexiEach, listOrNode, function(e) {
            var c;
            if (isList(e)) return clone(e); else if (isNode(e)) {
                c = e["cloneNode"](true);
  ...
  */
    ...
    function dollarRaw(selector, context, childOnly) {
        function filterElements(list) {
            // converts into array, makes sure context is respected
            var retList = collector(flexiEach, list, function flatten(a) {
                // flatten list, keep non-lists, remove nulls
                return isList(a) ? collector(flexiEach, a, flatten) : a;
            });
            if (parent) return filter(retList, function(node) {
                var a = node;
                while (a = a["parentNode"]) if (a == parent || childOnly) return a == parent;
            }); else return retList;
        }
        ...
        // note that context may have changed in the previous two lines!! you can't move this line
        if (!isString(selector)) return filterElements(selector);
        if (parent && isNode(parent) != 1) return [];
        if ((subSelectors = selector.split(/\s*,\s*/)).length > 1) return collectUniqNodes(subSelectors, function(ssi) {
            return dollarRaw(ssi, parent, childOnly);
        });
        if (steps = /(\S+)\s+(.+)$/.exec(selector)) return dollarRaw(steps[2], dollarRaw(steps[1], parent), childOnly);
        if (selector != (subSelectors = replace(selector, /^#/))) return filterElements(document.getElementById(subSelectors));
        ...
        if (regexpFilter = useGEbC ? elementName : className) elements = filter(elements, wordRegExpTester(regexpFilter, useGEbC ? "tagName" : "className"));
  ...
  */
    function promise() {
        var state;
        // undefined/null = pending, true = fulfilled, false = rejected
        var deferred = [];
        // this function calls the functions supplied by then()
        var assimilatedPromises = arguments;
        var assimilatedNum = assimilatedPromises.length;
        var numCompleted = 0;
        // number of completed, assimilated promises
        var values = [];
        // array containing the result arrays of all assimilated promises, or the result of the single promise
  ...
  */
   ...
   */
        text: function() {
            ...
            function extractString(e) {
                ...
                if (nodeType == 1) return collector(flexiEach, e["childNodes"], extractString); else if (nodeType < 5) // 2 is impossible (attribute), so only 3 (text) and 4 (cdata)..
                return e["data"]; else return _null;
            }
        ...
        get: function(spec, toNumber) {
            ...
            if (element) {
                if (isString(spec)) {
                    ...
                    var prefix = match[1];
                    var s;
                    if (getter[prefix]) s = getter[prefix](this, match[2]); else if (spec == "$") s = self["get"]("className"); else if (spec == "$$") {
                        // @condblock ie8compatibility
                        if (IS_PRE_IE9) s = element["style"]["cssText"]; else // @condend
                        s = self["get"]("@style");
                    } else if (spec == "$$slide") s = self["get"]("$height"); else if (spec == "$$fade" || spec == "$$show") {
                        if (self["get"]("$visibility") == "hidden" || self["get"]("$display") == "none") s = 0; else if (spec == "$$fade") {
                            s = // @condblock ie8compatibility
                            IS_PRE_IE9 ? isNaN(self["get"]("$filter", true)) ? 1 : self["get"]("$filter", true) / 100 : // @condend
                    ...
                    s = _window["pageXOffset"] != _null ? _window["pageXOffset"] : (document["documentElement"] || document["body"]["parentNode"] || document["body"])["scrollLeft"]; else if (spec == "$$scrollY") s = _window["pageXOffset"] != _null ? _window["pageYOffset"] : (document["documentElement"] || document["body"]["parentNode"] || document["body"])["scrollTop"]; else if (prefix == "$") {
                        // @condblock ie8compatibility 
                        if (!_window["getComputedStyle"]) s = (element["currentStyle"] || element["style"])[replace(match[2], /^float$/, "cssFloat")]; else // @condend
                        s = _window["getComputedStyle"](element, _null)["getPropertyValue"](replace(match[2], /[A-Z]/g, function(match2) {
                            return "-" + match2.toLowerCase();
                        }));
                    } else if (prefix == "@") s = element.getAttribute(match[2]); else s = element[match[2]];
   ...
   */
        set: function(name, value) {
            ...
            if (value !== undef) {
                ...
                var prefix = match[1];
                if (setter[prefix]) setter[prefix](this, match[2], value); else if (name == "$$fade") {
                    // @condblock ie8compatibility 
                    self["set"]({
                        $visibility: value ? "visible" : "hidden"
                    })["set"](IS_PRE_IE9 ? value < 1 ? {
                        $filter: "alpha(opacity = " + 100 * value + ")",
                        $zoom: 1
                    } : {
                        $filter: ""
                    } : // clear filter for opacity=1!!
                    {
                        $opacity: value
                    });
                } else if (name == "$$slide") {
                    ...
                    if (value) self["set"]({
                        ...
                        $display: function(oldVal) {
                            // set for 2nd time: now we get the stylesheet's $display
                    ...
                    // @condblock ie8compatibility 
                    if (IS_PRE_IE9) self["set"]("$cssText", value); else // @condend
                    self["set"]("@style", value);
                } else flexiEach(this, function(obj, c) {
                    var newValue = isFunction(value) ? value($(obj)["get"](name), c, obj) : value;
                    if (prefix == "$") {
                        if (match[2]) obj["style"][match[2]] = newValue; else {
                    ...
                    } else if (name == "$$scrollX") obj["scroll"](newValue, $(obj)["get"]("$$scrollY")); else if (name == "$$scrollY") obj["scroll"]($(obj)["get"]("$$scrollX"), newValue); else if (prefix == "@") {
                        if (newValue == _null) obj.removeAttribute(match[2]); else obj.setAttribute(match[2], newValue);
                    } else obj[match[2]] = newValue;
                });
            } else if (isString(name) || isFunction(name)) self["set"]("$", name); else eachObj(name, function(n, v) {
                self["set"](n, v);
            });
            return self;
        },
            ...
            return this["each"](function(e, index) {
                ...
                function appendChildren(c) {
                    if (isList(c)) flexiEach(c, appendChildren); else if (isFunction(c)) appendChildren(c(e, index)); else if (c != _null) {
                        // must check null, as 0 is a valid parameter 
   ...
   */
        dial: function(properties1, properties2, linf) {
            ...
            function getColorComponent(colorCode, index) {
                return /^#/.test(colorCode) ? parseInt(colorCode.length > 6 ? colorCode.substr(index * 2 + 1, 2) : (colorCode = colorCode.charAt(index + 1)) + colorCode, 16) : extractNumber(colorCode.split(",")[index]);
            }
            return function(t) {
                eachObj(properties1, function(name, start) {
                    var end = properties2[name], i = 0;
                    self["set"](name, t <= 0 ? start : t >= 1 ? end : /^#|rgb\(/.test(end) ? // color in format '#rgb' or '#rrggbb' or 'rgb(r,g,b)'?
                    "rgb(" + Math.round(interpolate(getColorComponent(start, i), getColorComponent(end, i++), t)) + "," + Math.round(interpolate(getColorComponent(start, i), getColorComponent(end, i++), t)) + "," + Math.round(interpolate(getColorComponent(start, i), getColorComponent(end, i++), t)) + ")" : replace(end, /-?[\d.]+/, toString(interpolate(extractNumber(start), extractNumber(end), t))));
                });
            };
        },
        /*$
   ...
   */
        values: function(data) {
            ...
            this["each"](function(el) {
                ...
                if (/form/i.test(el["tagName"])) // @condblock ie9compatibility 
                for (var i = 0; i < el["elements"].length; i++) // can't call directly, as IE<=9's elements have a nodeType prop and isList does not work
                $(el["elements"][i])["values"](r); else if (n && (!/ox|io/i.test(el["type"]) || el["checked"])) {
                    // ox|io => short for checkbox, radio
   ...
   */
        onOver: function(subSelect, toggle) {
            ...
            if (isFunction(toggle)) return this["on"](subSelect, "|mouseover |mouseout", function(ev, index) {
                // @condblock ie9compatibility 
            ...
            }); else return this["onOver"](_null, subSelect);
        },
        /*$
   * @id onfocus
   * @group EVENTS
   ...
   */
        onFocus: function(selector, handler, bubbleSelector) {
            if (isFunction(handler)) return this["on"](selector, "|blur", handler, [ false ], bubbleSelector)["on"](selector, "|focus", handler, [ true ], bubbleSelector); else return this["onFocus"](_null, selector, handler);
        },
        /*$
   * @id onchange
   * @group EVENTS
        ...
        onChange: function onChange(subSelect, handler, bubbleSelector) {
            if (isFunction(handler)) {
                ...
                return this["on"](subSelect, IS_PRE_IE9 ? "|propertychange |change |keyup |clicked" : "|input |change |clicked", function(ev, index) {
                    var e = this[0];
                    var v;
                    if (IS_PRE_IE9 && /select/i.test(e["tagName"])) v = e["options"][e["selectedIndex"]]["text"]; else v = /ox|io/i.test(e["type"]) ? e["checked"] : e["value"];
                    if (v != e[MINIFIED_MAGIC_PREV]) {
                        handler.call(this, e[MINIFIED_MAGIC_PREV] = v, index);
                    }
                }, bubbleSelector);
            } else return this["onChange"](_null, subSelect, handler);
        },
        /*$
   * @id onclick
   * @group EVENTS
   ...
   */
        onClick: function(subSelect, handler, args, bubbleSelector) {
            if (isFunction(handler)) return this["on"](subSelect, "click", handler, args, bubbleSelector); else return this["onClick"](_null, subSelect, handler, args);
        },
        /*$
   * @id trigger
   * @group EVENTS
   ...
   */
        trigger: function(eventName, eventObj) {
            return this["each"](function(element, index) {
                ...
                while (el && !stopBubble) {
                    flexiEach(IS_PRE_IE9 ? registeredEvents[el[MINIFIED_MAGIC_NODEID]] : el["M"], function(hDesc) {
                        if (hDesc.eventType == eventName) stopBubble = stopBubble || !hDesc.handlerFunc(eventObj, element);
                    });
                    el = el["parentNode"];
                }
            });
            ...
            try {
                ...
                xhr["onreadystatechange"] = function() {
                    if (xhr["readyState"] == 4 && !callbackCalled++) {
                        if (xhr["status"] == 200) prom(true, [ xhr["responseText"], xhr ]); else prom(false, [ xhr["status"], xhr["responseText"], xhr ]);
                    }
                };
                xhr["send"](data);
            } catch (e) {
                if (!callbackCalled) prom(false, [ 0, _null, toString(e) ]);
            }
            return prom;
        },
        /*
  ...
  */
        ...
        toJSON: function toJSON(value) {
            ...
            //result: "null"; toString(value) is not possible, because it returns an empty string for null
            if (isString(value = value.valueOf())) return '"' + replace(value, /[\\\"\x00-\x1f\u2028\u2029]/g, ucode) + '"';
            if (isList(value)) return "[" + collector(flexiEach, value, toJSON).join() + "]";
            if (isObject(value)) return "{" + collector(eachObj, value, function(k, n) {
                return toJSON(k) + ":" + toJSON(n);
            }).join() + "}";
  ...
  */
        ...
        parseJSON: _window.JSON ? _window.JSON.parse : function(text) {
            var t = replace(text, /[\x00\xad\u0600-\uffff]/g, ucode);
            // encode unsafe characters
            if (/^[[\],:{}\s]*$/.test(replace(replace(t, /\\["\\\/bfnrtu]/g), // remove all escapes
            /"[^"\\\n\r]*"|true|false|null|[\d.eE+-]+/g))) return eval("(" + t + ")");
        },
        // @condend
  ...
  */
        loop: function(paintCallback) {
            ...
            // 30 fps as fallback
            function raFunc(ts) {
                eachObj(animationHandlers, function(id, f) {
                    f(ts);
                });
                if (animationHandlerCount) requestAnim(raFunc);
            }
            function stop() {
   ...
   */
        setCookie: function(name, value, dateOrDays, dontEscape) {
            document.cookie = name + "=" + (dontEscape ? value : escape(value)) + (dateOrDays ? "; expires=" + (isObject(dateOrDays) ? dateOrDays : new Date(+new Date() + dateOrDays * 864e5)).toUTCString() : "");
        },
        /*$
   * @id getcookie
   * @group COOKIE
   ...
   */
        filterObj: function(obj, f, ctx) {
            var r = {};
            eachObj(obj, function(key, value) {
                if (f.call(ctx || obj, key, value)) r[key] = value;
            });
            return r;
        },
        /*$
  ...
  */
    ...
    if (IS_PRE_IE9) {
        ...
        document["attachEvent"]("onreadystatechange", function() {
            if (/^[ic]/.test(document["readyState"])) triggerDomReady();
        });
...
LEGO.featureDetects = function(window, document, $, logger) {
    "use strict";
    var test = document.createElement("div");
    var teststyle = test.style;
    var exports = {};
    var testProperties = function(suffix) {
        var testArr = [ "O" + suffix, "ms" + suffix, "webkit" + suffix, "Moz" + suffix, suffix.charAt(0).toLowerCase() + suffix.slice(1) ];
        return testArr;
    };
    var hasFlexBox = function() {
    ...
    var styleTest = function(styleAttr) {
        var testArr = testProperties(styleAttr);
        for (var i = 0; i < testArr.length; i++) {
            if (typeof teststyle[testArr[i]] != "undefined") {
                return true;
            }
        }
        return false;
    ...
    exports.init = function() {
        logger.log("hasTransform - " + styleTest("Transform"));
        logger.log("hasFlexbox - " + hasFlexBox());
        if (!styleTest("Transform")) {
            $("body").set("+lego-no-transform");
        }
        if (!styleTest("BoxShadow")) {
            $("body").set("+lego-no-boxshadow");
        }
        if (!hasFlexBox()) {
            $(".lego-global-header").set("+lego-no-flexbox");
            $(".lego-global-footer").set("+lego-no-flexbox");
        }
    };
    return exports;
}(this, this.document, LEGO.mini.$, LEGO.logger.getInstance());

LEGO.shouldDisable = function(feature) {
    "use strict";
    var html = LEGO.mini.$("html", undefined, true);
    var hasAttribute = html.get("@lego-disable-" + feature);
    return hasAttribute || hasAttribute === "";
};
/*
LEGO.settingsService = function($, undefined) {
    "use strict";
    var isValidSetting = function(value) {
        var testForValid = new RegExp("\\s*scriptviewmodel.*?");
        if (value === undefined || testForValid.test(value)) {
            return false;
        }
        return true;
    };
    var context = document.getElementsByTagName("html")[0].getAttribute("data-context") || "unknown";
    var fallbackSettings = {
        xlinkServiceUrl: "/scripts/Global/Json/xlink.html",
        surveyServiceUrl: "/scripts/Global/Json/survey.html",
        LEGOIdApiUrl: "/scripts/Global/Json/legoId.html",
        environment: "live",
        locale: "en-us",
        texts: {
            PrivacyPolicyLink: "/scripts/Global/Json/privacy.html",
            CookiePolicyLink: "/scripts/Global/Json/cookie.html",
            LegalTextService: "/scripts/Global/Json/legalTexts.html"
        },
        context: context,
        siteName: context,
        aboutUsUrl: "/",
        ...
        cacheApiUrl: "/",
        debug: true,
        showMarketSelectorInMobile: false,
        revision: "1.0.0.0"
    };
    var debug = "false";
    var showMarketSelectorInMobile = "false";
    var settings = {
        xlinkServiceUrl: "https://wwwsecure.lego.com/da-dk/globalnavigationservices/api/v1/xlink/en-us",
        surveyServiceUrl: "https://lc-www-live-s.legocdn.com:443/r/www/r/npssurvey/init",
        environment: "live",
        locale: "en-us",
        texts: {PrivacyPolicyLink:"https://www.lego.com/en-us/legal/legal-notice/privacy-policy",CookiePolicyLink:"https://www.lego.com/en-us/legal/cookieinfo",LegalTextService:"https://www.lego.com/en-us/legal/services/legal/legaltextservice.ashx"},
        context: context,
        siteName: context,
        aboutUsUrl: "https://www.lego.com/aboutus",
        legoComUrl: "https://wwwwsecure.lego.com",
        cacheApiUrl: "https://lc-www-live-s.legocdn.com:443/r/www/r/globalnavigationservices",
        apiUrl: "https://wwwsecure.lego.com/da-dk/globalnavigationservices",
        defaultApiUrl: "https://wwwsecure.lego.com/da-dk/globalnavigationservices",
        debug: debug === "true",
        // jshint ignore:line
        showMarketSelectorInMobile: showMarketSelectorInMobile === "true",
        // jshint ignore:line
        LEGOIdApiUrl: "https://lc-www-live-s.legocdn.com/r/account2/r/api/en-us/v2/lego.id.api.js",
        revision: "2.0.0.2034"
    };
    var exports = {};
    exports.get = function(settingsName) {
        var setting = settings[settingsName];
        if (isValidSetting(setting)) {
            return setting;
        }
        return fallbackSettings[settingsName];
    };
    exports.set = function(settingsName, value) {
        settings[settingsName] = value;
    };
    return exports;
}(LEGO.mini.$);
*/
LEGO.eventBroadcastService = function(window, document, $, logger) {
    "use strict";
    ...
    var exports = {};
    exports.sendEvent = function(eventName) {
        eventName = eventName.toLowerCase();
        var event = createEvent(eventName);
        if (event === null) {
            logger.log("Could not create a custom event event for xlink.");
            return;
        }
...
LEGO.templates = function() {
    "use strict";
    var exports = {};
    exports.xlinkTemplate = '<div class="lego-modal-xlink lego-fade" role="dialog">' + '<div class="lego-modal-dialog">' + '<div class="lego-modal-content">' + '<div class="lego-modal-body lego-modal-grid" >' + '<div class="lego-modal-grid-row">' + '<figure class="lego-modal-grid-column lego-modal-grid-column-figure">' + '<img src="{{Image}}">' + "</figure>" + '<article class="lego-modal-grid-column">' + "<h2>{{Title}}</h2>" + "<p>{{Message}}</p>" + '<div class="lego-modal-buttons xlink">' + '<a href="#" class="lego-modal-close-trigger">{{CloseButtonText}}</a>' + '<a href="{{TargetUrl}}" target="{{Target}}" class="lego-modal-close-trigger lego-modal-redirect">{{ContinueButtonText}}</a>' + "</div>" + "</article>" + "</div>" + "</div>" + '<button type="button" class="lego-modal-close lego-modal-close-trigger">&times;</button>' + "</div>" + "</div>" + "</div>";
    exports.privacyPolicy = '<div class="lego-modal-xlink lego-fade lego-modal-privacypolicy" role="dialog">' + '<div class="lego-modal-dialog">' + '<div class="lego-modal-content">' + '<div class="lego-modal-body" >' + '<iframe src="{{TargetUrl}}" frameborder="0" scrolling="no"></iframe>' + "</div>" + '<button type="button" class="lego-modal-close lego-modal-close-trigger">&times;</button>' + "</div>" + "</div>" + "</div>";
    return exports;
}();

LEGO.modal = function(window, document, logger, $, EE, _, templates, broadcast, undefined) {
    "use strict";
    var instance;
    var modal;
    var backdrop;
    ...
    var createInstance = function() {
        ...
        var create = function(htmlStr) {
            var content = EE("div");
            content.set("$", "lego-modal-wrapper lego-modal-close-trigger");
            content.set("innerHTML", htmlStr);
            return content;
        };
        ...
        var setStyling = function() {
            backdrop = $(".lego-modal-wrapper");
            body = $("body");
            body.set("$", "lego-modal-open");
            backdrop.set("$", "lego-in");
            dialog.set("$", "lego-in");
        };
        ...
        var createModal = function(data, template, broadcastType) {
            if (!data) {
                logger.log("No data was send to modal", this);
                return;
            }
            if (modal) {
                exports.removeAll();
            ...
            var modalTemplate = _.template(template);
            var modalString = modalTemplate(data);
            modal = create(modalString);
            $("body").add(modal);
            dialog = $(".lego-modal-xlink");
            dialog.set({
                $display: "block"
            });
            $(".lego-modal-xlink").on("click", function(e) {
                e.stopPropagation();
            });
            $(".lego-modal-close-trigger").on("click", function(e) {
                if (modal) {
                    var temp = $(modal);
                    modal = undefined;
                    removeStyling();
                    ...
                    if (this.is(".lego-modal-redirect")) {
                        e.preventDefault();
                        var url = decodeURIComponent(this.get("@href"));
                        broadcast.sendEvent(broadcastType + "continue");
                        var target = this.get("@target");
                        if (target && target.indexOf("blank") > -1) {
                            window.open(url, "_blank");
                        } else {
                            location.href = url;
                        }
                    } else {
                        broadcast.sendEvent(broadcastType + "cancel");
                    }
                }
            });
            setTimeout(function() {
        ...
        exports.createIframeModal = function(data) {
            createModal(data, templates.privacyPolicy);
            return;
        };
        exports.createXlinkModal = function(data, broadcastType) {
            createModal(data, templates.xlinkTemplate, broadcastType);
            return;
        };
        return exports;
    };
...
}(this, this.document, LEGO.logger.getInstance(), LEGO.mini.$, LEGO.mini.EE, LEGO.mini._, LEGO.templates, LEGO.eventBroadcastService);

LEGO.xlink = function(window, document, $, logger, modal, broadcast, settings) {
    "use strict";
    var exports = {};
    var pattern = /(\?|&)LEGOXLinked\=(.{38})/;
    var serviceUrl = "/api/v1/xlink/";
    ...
    var isShopXlink = function(href) {
        var culture = settings.get("locale");
        return !!href && (href.toLowerCase().indexOf("#shopxlink") > -1 && culture.toLowerCase() === "en-us");
    };
    var isXlink = function(href) {
        return isShopXlink(href) || href.toLowerCase().indexOf("#xlink") > -1 || href.toLowerCase().indexOf("legoxlinked") > -1;
    };
    var getPageId = function(href) {
        return !!href && pattern.exec(href)[2];
    };
    var getData = function(href, target) {
        // don't do anything if there is no href
        if (!href) {
            return {};
        }
        if (href.indexOf("LEGOXLinked") > 0) {
            return {
                pageid: getPageId(href),
                href: "",
                returnurl: window.encodeURIComponent(exports.location.href),
                target: target,
                type: "xlink",
                eventPrefix: "xlink"
            };
        }
        if (href.indexOf("#xlink") > 0) {
            return {
                pageid: "",
                href: window.encodeURIComponent(href.replace(/#xlink/, "")),
                returnurl: window.encodeURIComponent(exports.location.href),
                target: target,
                type: "xlink",
                eventPrefix: "xlink"
        ...
        if (href.indexOf("#shopxlink") > 0) {
            return {
                pageid: "",
                href: window.encodeURIComponent(href.replace(/#shopxlink/, "")),
                returnurl: window.encodeURIComponent(exports.location.href),
                target: target,
                type: "XLinkShop",
                eventPrefix: "XLinkShop"
            };
        }
        return false;
    };
    var fixServiceUrlForLegacy = function(url) {
        if ("withCredentials" in new XMLHttpRequest()) {
            return url;
        } else {
            if (typeof XDomainRequest !== "undefined") {
                if (location.href.indexOf("https://") < 0) {
                    return url.replace("https", "http").replace("wwwsecure", "www");
                }
            }
        }
        return url;
    ...
    var generateModal = function(data, eventPrefix) {
        var response = JSON.parse(data);
        modal.createXlinkModal(response, eventPrefix);
        broadcast.sendEvent(eventPrefix + "open");
    };
    exports.createModal = function(serviceMethod, url, params) {
        if (window.XDomainRequest) {
            var xdr = new XDomainRequest();
            var serviceUrl = url + "?pageid=" + params.pageid + "&href=" + params.href + "&returnurl=" + params.returnurl + "&target=" + params.target + "&type=" + params.type + "&eventPrefix=" + params.eventPrefix;
            xdr.open(serviceMethod, serviceUrl);
            xdr.onload = function() {
                generateModal(xdr.responseText, params.eventPrefix);
            };
            xdr.onerror = function() {
                logger.log(xdr.responseText);
            };
            setTimeout(function() {
                xdr.send();
            }, 0);
        ...
        $.request(serviceMethod, url, params, {
            headers: {
                Accept: "application/json, text/html"
            }
        }).then(function(data) {
            generateModal(data, params.eventPrefix);
        }).error(function(status, statusText, responseText) {
            logger.log(status);
    ...
    exports.configure = function() {
        if (LEGO.shouldDisable("xlink")) {
            logger.log("xlink disable");
            return exports;
        }
        $(document).on("?click", function() {
            // do not process if there is no href
            var href = this.get("@href");
            if (!href) {
                return true;
            }
            var params = getData(href, this.get("@target") === null ? "" : this.get("@target"));
            var eventPrefix = this.get("@data-event-prefix");
            if (eventPrefix) {
                params.eventPrefix = eventPrefix;
            }
            if (!isXlink(href)) {
                broadcast.sendEvent(params.eventPrefix + "directopen");
                return true;
            }
            var url = fixServiceUrlForLegacy(serviceUrl);
            exports.createModal(serviceMethod, url, params);
            return false;
        }, "a");
    };
    return exports;
}(this, this.document, LEGO.mini.$, LEGO.logger.getInstance(), LEGO.modal.getInstance(), LEGO.eventBroadcastService, LEGO.settingsService);

LEGO.detection = function(ua) {
    "use strict";
    var self = {};
    var useragent = ua;
    ...
    if (useragent) {
        ...
        var getMinorVersion = function() {
            ...
            if (self.browser.isIE && version >= 4) {
                version = useragent.substring(useragent.indexOf("msie ") + 5, useragent.indexOf("msie ") + 8);
            } else if (self.browser.isSafari) {
                version = parseFloat(useragent.substring(useragent.lastIndexOf("safari/") + 7));
            }
            return version;
        };
        self.browser.versionMinor = getMinorVersion();
        self.browser.versionMajor = parseInt(getMinorVersion());
    }
    ...
    self.isTablet = function() {
        return self.checks.ipad || self.checks.tabletPc || self.checks.playbook || self.checks.androidTablet || self.checks.kindle;
    };
    self.isSmartPhone = function() {
        return (self.checks.mobile || self.checks.blackberry || self.checks.palmDevice || self.checks.otherMobileHints) && !self.isTablet() && !self.checks.ipod;
    };
    return self;
}(navigator.userAgent || navigator.vendor || window.opera);

/*! Cookies.js - 0.4.0; Copyright (c) 2014, Scott Hamper; http://www.opensource.org/licenses/MIT */
/* LEGO CUSTOM - The "definer" from the original author has been scrapped to assure always returning LEGO.Cookies instead */
LEGO = this.LEGO || {};
...
LEGO.NpsSurvey = function(window, $, logger, settingsService) {
    "use strict";
    var exports = {};
    exports.init = function() {
        if (LEGO.shouldDisable("nps-survey")) {
            logger.log("nps survey disable");
            return exports;
        }
        var npsLocale = settingsService.get("locale");
        var npsContext = document.getElementsByTagName("html")[0].getAttribute("data-context") || "unknown";
        var npsOverwrite = document.getElementsByTagName("body")[0].getAttribute("data-lego-nps");
        var mobileState = (window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) < 1024;
        var npsSurveyId = mobileState ? 121455 : npsOverwrite ? npsOverwrite : 104172;
        var npsDisplayMode = mobileState ? "mobile" : "desktop";
        //TODO - should we handle this, only used for service context??
        if (window.location.protocol.indexOf("https") > -1 && npsContext === "service") {
            return false;
        } else {
            var script = document.createElement("script");
            script.type = "text/javascript";
            script.async = true;
            script.src = settingsService.get("surveyServiceUrl") + "?context=" + npsContext + "&locale=" + npsLocale + "&surveyid=" + npsSurveyId + "&displaymode=" + npsDisplayMode;
            document.getElementsByTagName("head")[0].appendChild(script);
        }
        return exports;
    };
    return exports;
}(window, LEGO.mini.$, LEGO.logger.getInstance(), LEGO.settingsService);

// ReSharper disable UnknownCssClass
// ReSharper disable once InconsistentNaming
LEGO.PrivacyPolicy = function(window, document, $, EE, logger, deviceDetect, modal, settingsService, Cookies, undefined) {
    "use strict";
    Cookies.defaults = {
        domain: "lego.com"
    };
    ...
    var exports = {};
    var body = "body";
    var currWasHere = "visited";
    var container = "#GFSticky";
    var speechbubble = container + " .speech-bubble";
    var currState = "opened";
    var text = {
        PrivacyPolicy: "",
        PrivacyPolicyLink: "?",
        CookiePolicyLink: "?",
        NotifyOnceText: "",
        Cookies: "",
        LearnMoreAboutCookiesLink: ""
    };
    var hideEmbed = function() {
        var hideObjs = document.getElementsByTagName("embed");
        for (var i = 0; i < hideObjs.length; i += 1) {
    ...
    var create = function() {
        if (Cookies.get("cps") === undefined) {
            Cookies.set("cps", "true", {
                expire: 400
            });
            currWasHere = "first-time-visit";
        }
        var ieVersion = deviceDetect.isIE ? deviceDetect.browser.versionMinor : 0;
        if (deviceDetect.isIE) {
            var addIeClass = "l-gfs-ie" + ieVersion;
            $(body).set(addIeClass);
        }
        var size;
        if (getGfSticky() !== undefined) {
            getGfSticky().remove();
        }
        doMarkUp();
        var ppLinkEle = $("#GFSpplink");
        var ciLinkEle = $("#GFScilink");
        var firstTimeUsePopup = $("#first-time-use-link");
        var closeInfo = $("#closing");
        applyCss();
        ppLinkEle.on("click", function() {
            var targetUrl = text.PrivacyPolicyLink;
            size = window.getComputedStyle ? window.getComputedStyle(document.body, ":after").getPropertyValue("content") : "mobile";
            createModal(targetUrl, size);
        });
        ciLinkEle.on("click", function() {
            hideEmbed();
            var targetUrl = text.CookiePolicyLink;
            size = window.getComputedStyle ? window.getComputedStyle(document.body, ":after").getPropertyValue("content") : "mobile";
            createModal(targetUrl, size);
        });
        firstTimeUsePopup.on("click", function() {
            hideEmbed();
            var targetUrl = text.CookiePolicyLink;
            createModal(targetUrl, size);
        });
    ...
    var doMarkUp = function() {
        //Fixed Footer
        var gfSticky = EE("div");
        gfSticky.set("@id", container.slice(1));
        $(body).add(gfSticky);
        gfSticky.set("$", settingsService.get("locale"));
        gfSticky.set("$", currWasHere);
        if (deviceDetect.isTouchDevice()) {
            gfSticky.set("$", "touch");
        }
        //List of links
        var linkList = EE("ul");
        gfSticky.add(linkList);
        //Privacy Policy
        var ppItem = EE("li");
        linkList.add(ppItem);
        var ppLink = EE("a");
        ppLink.set("@id", "GFSpplink");
        ppItem.add(ppLink);
        ppLink.set("@href", text.PrivacyPolicyLink);
        ppLink.set("innerHTML", text.PrivacyPolicy);
        //Markup for first time use
        var spanText = EE("span");
        spanText.set("$", "extended-info");
        var closeBtn = EE("span");
        closeBtn.set("$", "close-btn");
        closeBtn.set("@id", "closing");
        gfSticky.add(closeBtn);
        gfSticky.add(spanText);
        spanText.set("innerHTML", text.NotifyOnceText + " ");
        var linkText = EE("a");
        linkText.set("@id", "first-time-use-link");
        linkText.set("@href", text.CookiePolicyLink);
        linkText.set("innerHTML", text.LearnMoreAboutCookiesLink);
        spanText.add(linkText);
        //Cookie Info
        var ciItem = EE("li");
        linkList.add(ciItem);
        var ciLink = EE("a");
        ciLink.set("@id", "GFScilink");
        ciLink.set("@href", text.CookiePolicyLink);
        ciLink.set("innerHTML", text.Cookies);
        ciItem.add(ciLink);
        return {
            closeBtn: closeBtn,
            firstTimeSpan: spanText
        };
    };
    var closing = function() {
        var gfSticky = getGfSticky();
        gfSticky.set("-first-time-visit");
        gfSticky.set("+visited");
        showEmbed();
    };
    var applyCss = function() {
        var gfSticky = getGfSticky();
        gfSticky.set("-opened");
        gfSticky.set("-collapsed");
        gfSticky.set("$", currState);
        if (deviceDetect.isTouchDevice()) {
            gfSticky.set("$", "touch-device");
        }
        var collapsed = currState == "collapsed";
        exports.cssbubbletext = collapsed ? text.maximize : text.minimize;
        exports.csstext = collapsed ? "&#x25BC;" : "&#x25B2;";
        var bubble = $(speechbubble);
        if (bubble !== undefined && bubble !== null) {
            bubble.set("inneHTML", exports.cssbubbletext);
        }
    };
    ...
    var setText = function(textObject, callback) {
        loadJSONP(textObject.LegalTextService, function(response) {
            text.NotifyOnceText = response.NotifyOnceText;
            text.Cookies = response.Cookies;
            text.LearnMoreAboutCookiesLink = response.LearnMoreAboutCookiesLink;
            text.PrivacyPolicy = response.PrivacyPolicy;
            text.CookiePolicyLink = textObject.CookiePolicyLink;
            text.PrivacyPolicyLink = textObject.PrivacyPolicyLink;
            callback();
        });
    };
    var createModal = function(targetUrl, size) {
        var options;
        switch (size) {
          case "mobile":
          ...
          case "tablet":
            ...
            modal.createIframeModal(options);
            break;
    ...
    exports.init = function() {
        if (LEGO.shouldDisable("privacy-policy")) {
            logger.log("privacy policy disable");
            return exports;
        }
        logger.log("creating privacy policy");
        setText(settingsService.get("texts"), create);
        return exports;
    };
    return exports;
}(this, this.document, LEGO.mini.$, LEGO.mini.EE, LEGO.logger.getInstance(), LEGO.detection, LEGO.modal.getInstance(), LEGO.settingsService, LEGO.Cookies);

LEGO.trackingService = function(logger) {
    "use strict";
    var exports = {};
    exports.legacytrackClick = function(mainSection, clickAction) {
        var site = "portal";
        try {
            if (!TrackMan) {
                logger.log("TrackMan is not present, cannot do tracking");
                return;
            }
            TrackMan.useModule([ TrackMan.Modules.BasicTracking ], function success(basicTracking) {
                logger.log("" + site + ":" + mainSection + ":" + clickAction);
                basicTracking.setChannel(site, mainSection).clickAction(clickAction);
            });
        } catch (e) {
            logger.log("TrackMan is not present, cannot do tracking - catch exception");
        }
    };
    exports.globalHeaderClick = function(clickAction) {
        try {
            if (!TrackMan) {
                logger.log("TrackMan is not present, cannot do tracking");
                return;
            ...
            TrackMan.useModule([ TrackMan.Modules.BasicTracking ], function success(basicTracking) {
                logger.log("Global header click-action:" + clickAction);
                basicTracking.clickAction("global-header-" + clickAction);
            });
        } catch (e) {
            logger.log("TrackMan is not present, cannot do tracking - catch exception");
        }
    ...
    exports.init = function() {
        logger.log("tracking init...");
        return exports;
    };
    return exports;
}(LEGO.logger.getInstance());

/* global LEGO */
LEGO.navigation = function(window, document, trackingService, $, $$, LEGOGlobal) {
    "use strict";
    var exports = {};
    // if there is no computedstyle we'll take that to be mobile state
    var mobileState = !window.getComputedStyle;
    if (!mobileState) {
        // find the hidden-breakpoint-observer element and check the visibility (if it is there)
        var hiddenBreakpointObserver = window.getComputedStyle($(".hidden-breakpoint-observer")[0]);
        if (hiddenBreakpointObserver) {
            mobileState = hiddenBreakpointObserver.getPropertyValue("visibility") !== "hidden";
        }
    }
    //normalization of url
    var normalize = function(item) {
        // make sure that wwwsecure is normalized to www - use the hostname so we don't get port number
        var domain = item.hostname.replace("wwwsecure.", "www.");
        // remove trailing slash
        var path = domain + item.pathname.replace(new RegExp("[/]+$"), "");
        path = path.toLowerCase();
        return path;
    };
    var globalNavigationtrackingPreName = "navigation-";
    var sendGlobalHeaderLinkTracking = function(linkName) {
        trackingService.globalHeaderClick(globalNavigationtrackingPreName + linkName);
    };
    var globalNavigationEventsFired = {};
    var onNavigationClick = function(clickEvent) {
        var classList = clickEvent.srcElement.classList.value;
        // Get link-type from icon class
        if (classList) {
            var poss = classList.indexOf("gn-icon-");
            var elementClass = "";
            if (poss >= 0) {
                elementClass = classList.substring(poss + 8);
                if (elementClass.indexOf(" ") >= 0) {
                    elementClass = elementClass.substring(0, elementClass.indexOf(" "));
                }
            } else {
                poss = classList.indexOf("shop");
                if (poss >= 0) {
                    elementClass = "shop";
                }
            }
            if (elementClass) {
                if (globalNavigationEventsFired[elementClass]) {
                    // Prevent same clickevent from looping
                    LEGOGlobal.trackingClick = false;
                    return;
                }
                globalNavigationEventsFired[elementClass] = true;
                // Make sure same event is not fired recursively - and only one track record per link-click
                LEGOGlobal.trackingClick = true;
                sendGlobalHeaderLinkTracking(elementClass);
            }
        }
    };
    function delay(ms) {
    ...
    var onWindowUnload = function() {
        if (LEGOGlobal.trackingClick) {
            delay(250);
        }
    };
    var setupUnloadEvent = function() {
        var existingOnUnloadHandler = window.onbeforeunload;
        window.onbeforeunload = function(unloadEvent) {
            if (existingOnUnloadHandler) {
                existingOnUnloadHandler(unloadEvent);
            }
            onWindowUnload();
            return;
        };
    ...
    exports.init = function() {
        var globalHeader = $(".lego-global-header");
        var globalDropDown = $(".collapse-wrapper", globalHeader);
        var globalNavigation = $(".lego-global-navigation", globalHeader);
        var globalNavigationButton = $(".logo", globalHeader);
        var globalNavigationShroud = $(".shroud", globalHeader);
        var siteNavigation = $(".site-navigation");
        var firstLegoBrandOpen = true;
        var toggleNavigation = function() {
            var newState = globalNavigationButton.get("@data-state") == "open" ? "closed" : "open";
            if (newState === "open" && firstLegoBrandOpen) {
                firstLegoBrandOpen = false;
                trackingService.globalHeaderClick(globalNavigationtrackingPreName + "legologodropdown");
            }
            globalNavigationButton.set({
                "@data-state": newState
            });
            globalNavigationShroud.set({
                "@data-state": newState
            });
            globalDropDown.set({
                "@data-state": newState
        ...
        var toggleTabIndex = function() {
            var newState = globalNavigationButton.get("@data-state") == "open" ? "" : "-1";
            $("a, select", globalDropDown).each(function(item) {
                item.tabIndex = newState;
            });
        };
        if (siteNavigation && siteNavigation.length > 0) {
            var fragment = siteNavigation.clone();
            var persistedClasses = [ "^icon", "^active$" ];
            $("*", fragment).each(function(item) {
                var classes = item.className.split(" ");
                var newClass = "";
                for (var i = 0, il = classes.length; i < il; i++) {
                    var classCount = persistedClasses.length;
                    while (classCount--) {
                        if (classes[i].match(persistedClasses[classCount])) {
                            newClass += classes[i] + " ";
                        }
                    }
                }
            ...
            $$(fragment).className = "site-navigation-cloned";
            globalNavigation.add(fragment);
        }
        // iterate all a-tags in global links element
        var links = $(".global-links", globalNavigation);
        if (links && links.length > 0) {
            // normalize the current location
            var currentPath = normalize(document.location);
            setupUnloadEvent();
            $("a", links).each(function(item) {
                var path = normalize(item);
                var isActive = path === currentPath;
                // if there is a match add active class to the a-tag
                if (isActive) {
        ...
        //remove links from tabindex if not on desktop and expanded states
        if (!(LEGO.expanded && !mobileState)) {
            var initTabState = function() {
                toggleTabIndex();
            };
            globalNavigationButton.on("focus", initTabState);
        }
        // Avoid ghostclick on Android 4.2
        var handleEvents = navigator.userAgent.toLowerCase().indexOf("android 4.2.") === -1 ? "click touchstart" : "click";
        globalNavigationButton.on(handleEvents, function(e) {
            e.preventDefault();
            if (!mobileState) {
                LEGOGlobal.trackingClick = true;
                // Make sure the tracking event is fired, before unloading page
                trackingService.globalHeaderClick(globalNavigationtrackingPreName + "legologo");
                window.location = globalNavigationButton.get("@href");
                return;
            }
            toggleNavigation();
            //IE8 hack for UI repaint
        ...
        globalNavigationShroud.on("click", function() {
            toggleNavigation();
            //IE8 hack for UI repaint
            if (window.attachEvent) {
                globalDropDown.set({
    ...
    return exports;
}(this, this.document, LEGO.trackingService, LEGO.mini.$, LEGO.mini.$$, LEGO.Global = LEGO.Global || {});

LEGO.globalsearch = function(window, $, settingsService, trackingService, logger) {
    "use strict";
    var exports = {};
    exports.init = function() {
        var globalHeader = $(".lego-global-header");
        if (LEGO.shouldDisable("search")) {
            logger.log("search disable");
            globalHeader.set("$", "+hide-search");
            return exports;
        }
        var searchButton = $(".lego-global-search-submit", globalHeader);
        var searchUrl = "https://search2.lego.com/";
        var searchField = $(".lego-global-search-field", globalHeader);
        var searchOnEnter = false;
        if (searchField && searchField.length) {
            searchField.each(function(item) {
                item.onkeypress = function(keyinfo) {
        ...
        searchButton.on("click", function(event) {
            trackingService.globalHeaderClick("search-" + (searchOnEnter ? "fieldenterpressed" : "button"));
            event.preventDefault();
            var searchInput = $(".lego-global-search-field", this[0].parentNode);
            var locale = settingsService.get("locale");
            var shopLocale = locale.substring(0, 3) + locale.slice(3).toUpperCase();
            searchUrl = searchUrl + shopLocale;
    ...
    return exports;
}(this, LEGO.mini.$, LEGO.settingsService, LEGO.trackingService, LEGO.logger.getInstance());

LEGO.marketSelector = function(window, document, $, $$, settingsService, trackingService, _, logger, undefined) {
    "use strict";
    var exports = {};
    var Url = function() {
        var removeCultureFromPath = function(path) {
            var cultureReplaceRegex = new RegExp("/[a-z]{2}-[a-z]{2}(/|$)|/en(/|$)", "i");
            return path.replace(cultureReplaceRegex, "/");
        };
        var ensurePrefixSlash = function(path) {
            return path.replace(/^\/?/, "/");
        };
        function Url(protocol, host, culture, path, query, hash) {
            this.protocol = protocol.replace(":", "");
            this.host = host;
            this.culture = ensurePrefixSlash(culture.toLowerCase());
            var pathname = removeCultureFromPath(path);
            this.path = ensurePrefixSlash(pathname);
            this.hash = hash.replace(/^#/, "");
            query = query.replace(/^\?/, "");
            this.query = new QueryString(query);
        }
        Url.prototype.toString = function() {
            return (this.protocol && this.protocol + "://") + (this.host && this.host) + (this.culture && this.culture) + (this.path && this.path) + (this.query.toString() && "?" + this.query) + (this.hash && "#" + this.hash);
        };
    ...
    var sendMarketChangeTracking = function() {
        trackingService.globalHeaderClick("marketselector-changemarket");
        trackingService.legacytrackClick("marketselectorfullmarket", "changemarket");
    };
    exports.generateMarketUrl = function(culture, isDynamic) {
        // Ensure location is not updated in test
        if (isDynamic) {
            updateLocation();
        }
        var currentLocation = exports.location;
        var url = new Url(currentLocation.protocol, currentLocation.host, culture, currentLocation.pathname, currentLocation.search, currentLocation.hash);
        url.query.ignorereferer = true;
        return url.toString();
    };
    exports.init = function() {
        var globalHeader = $(".lego-global-header");
        if (LEGO.shouldDisable("marketselector")) {
            logger.log("marketselector disable");
            globalHeader.set("$", "+hide-marketselector");
            return exports;
        }
        var marketSelector = $(".market-selector", globalHeader);
        if (marketSelector && marketSelector.length) {
            var hasDroppedDown = false;
            marketSelector.each(function(item) {
                item.addEventListener("focus", function() {
                    if (!hasDroppedDown) {
                        hasDroppedDown = true;
                        trackingService.globalHeaderClick("marketselector-open");
                    }
                });
            });
            marketSelector.on("change", function() {
                logger.log("redirecting to " + $$(this).value);
                sendMarketChangeTracking();
                location.href = exports.generateMarketUrl($$(this).value, true);
            });
        }
        return exports;
    };
    return exports;
}(this, this.document, LEGO.mini.$, LEGO.mini.$$, LEGO.settingsService, LEGO.trackingService, LEGO.mini._, LEGO.logger.getInstance());

window.LEGO.ID = function($, settingService, logger) {
    "use strict";
    var exports = {};
    exports.hasLegoId = function() {
        var scripts = $("script");
        for (var i = 0; i < scripts.length; i++) {
            var script = scripts[i];
            var scriptSrc = script.getAttribute("src");
            if (scriptSrc) {
                if (scriptSrc.indexOf("lego.id.header.js") > -1) {
                    return true;
                }
            }
        }
    ...
    exports.init = function() {
        if (LEGO.shouldDisable("lego-id")) {
            logger.log("lego-id disable");
            return exports;
        }
        window.LEGOIDReady = window.LEGOIDReady || [];
        window.LEGOIDReady.push(function() {
            window.LEGOID.API.loadBox({
                selector: ".legoid-box",
                scope: window,
                callback: function() {}
            });
        });
        var path = settingService.get("LEGOIdApiUrl");
        (function(d, path) {
    ...
    return exports;
}(LEGO.mini.$, LEGO.settingsService, LEGO.logger.getInstance());

LEGO.Global = function(xlink, nps, privacy, navigation, logger, settingsService, loggerService, id, marketSelector, featureDetects, globalsearch, undefined) {
    "use strict";
    var exports = {};
    exports.loaded = false;
    var tryCatcher = function(callback, success, error) {
    ...
    exports.init = function() {
        ...
        exports.loaded = true;
        if (featureDetects !== undefined) {
            var featureDetectSuccess = function() {
                logger.log("loaded feature detects succesfully");
            };
            var featureDetectError = function(err) {
                logger.log("loading feature detects failed");
                logger.log(err);
            };
            tryCatcher(featureDetects.init, featureDetectSuccess, featureDetectError);
        }
        if (id !== undefined) {
            if (!id.hasLegoId()) {
                var successLegoId = function() {
                    logger.log("loaded LEGOId succesfully");
                };
                var errorLegoId = function(err) {
                    logger.log("loading LEGOId failed");
                    logger.log(err);
                };
                tryCatcher(id.init, successLegoId, errorLegoId);
            }
        }
        /*if (xlink !== undefined) {
            var serviceUrl = settingsService.get("xlinkServiceUrl");
            var xlinkerSuccess = function() {
                logger.log("loaded xlink succesfully");
                logger.log("enabling xlink - " + serviceUrl);
                xlink.setUrl(serviceUrl);
            };
            var xlinkerError = function(err) {
                logger.log("loading xlink failed");
                logger.log(err);
            };
            tryCatcher(xlink.configure, xlinkerSuccess, xlinkerError);
        }*/
        if (nps !== undefined) {
            var surveySuccess = function() {
                logger.log("loaded nps succesfully");
            };
            var surveyError = function(err) {
                logger.log("loading nps failed");
                logger.log(err);
            };
            tryCatcher(nps.init, surveySuccess, surveyError);
        }
        if (privacy !== undefined) {
            var privacyPolicySuccess = function() {
                logger.log("loaded privacy footer succesfully");
            };
            var privacyPolicyError = function(err) {
                logger.log("loading privacy footer failed");
                logger.log(err);
            };
            tryCatcher(privacy.init, privacyPolicySuccess, privacyPolicyError);
        }
        if (navigation !== undefined) {
            var globalNavigationSuccess = function() {
                logger.log("loaded navigation succesfully");
            };
            var globalNavigationError = function(err) {
                logger.log("loading navigation failed");
                logger.log(err);
            };
            tryCatcher(navigation.init, globalNavigationSuccess, globalNavigationError);
        }
        if (marketSelector !== undefined) {
            var marketSuccess = function() {
                logger.log("loaded marketSelector succesfully");
            };
            var marketError = function(err) {
                logger.log("loading marketSelector failed");
                logger.log(err);
            };
            tryCatcher(marketSelector.init, marketSuccess, marketError);
        }
        if (globalsearch !== undefined) {
            var globalsearchSuccess = function() {
                logger.log("loaded globalsearch succesfully");
            };
            var globalsearchError = function(err) {
                logger.log("loading globalsearch failed");
                logger.log(err);
            };
            tryCatcher(globalsearch.init, globalsearchSuccess, globalsearchError);
        }
    };
    ...
    return exports;
}(LEGO.xlink, LEGO.NpsSurvey, LEGO.PrivacyPolicy, LEGO.navigation, LEGO.logger.getInstance(), LEGO.settingsService, LEGO.logger, LEGO.ID, LEGO.marketSelector, LEGO.featureDetects, LEGO.globalsearch);